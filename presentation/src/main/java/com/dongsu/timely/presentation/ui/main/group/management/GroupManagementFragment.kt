...
class GroupManagementFragment: BaseTabFragment<FragmentGroupManagementBinding>(FragmentGroupManagementBinding::inflate) {
    ...
    private fun createInviteCode() {
        lifecycleScope.launch {
            val result = groupManagementViewModel.createInviteCode(groupId)
            when(result){
                is TimelyResult.Success -> {
                    with(result.resultData){
                        sendInviteMessage(groupId,inviteCode)
                    }
                }
    ...
    private fun sendInviteMessage(groupId: Int, inviteCode: String){

            val inviteTextTemplate = TextTemplate(
            text = INVITE_GROUP_MESSAGE
                .trimIndent(),
            link = Link(
                androidExecutionParams = mapOf(GROUP_ID to groupId.toString(), INVITE_CODE to inviteCode)
            )
        )
            // 카카오톡 설치여부 확인
            if (ShareClient.instance.isKakaoTalkSharingAvailable(requireContext())) {
                ...
                ShareClient.instance.shareDefault(requireContext(), inviteTextTemplate) { sharingResult, error ->
                    if (error != null) {
